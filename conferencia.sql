...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarUsuario`(IN `paramId` INT, IN `paramNombre` VARCHAR(25), IN `paramContra` VARCHAR(100), IN `paramMail` VARCHAR(50), IN `paramIdEmp` INT, IN `paramFoto` VARCHAR(50))
    NO SQL
